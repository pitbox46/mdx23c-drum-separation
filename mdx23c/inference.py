...
def demix(
    ...
    if mode == "demucs":
        ...
        chunk_size = getattr(
            config.inference,
            "chunk_size",
            getattr(config.audio, "chunk_size", mix.shape[-1]),
        )
    ...
    instruments = (
        ...
        if mode == "demucs"
        else prefer_target_instrument(config)
    )
...
def load_model(
    ...
    if state_dict is not None:
        ...
        try:
            ...
            if missing:
                print(
                    f"Warning: {len(missing)} missing keys (config mismatch likely). Example: {missing[:5]}"
                )
            if unexpected:
                print(
                    f"Warning: {len(unexpected)} unexpected keys in state dict. Example: {unexpected[:5]}"
                )
        ...
        try:
            ...
            unexpected = getattr(load_res, "unexpected_keys", [])
            num_loaded = len(
                [k for k in loaded_state_dict.keys() if k not in unexpected]
            )
            total_params = len(list(model.state_dict().keys()))
            print(
                f"Loaded checkpoint: {checkpoint_path} | loaded {num_loaded}/{total_params} tensors"
            )
            if missing:
                print(
                    f"Warning: {len(missing)} missing keys (config mismatch likely). Example: {missing[:5]}"
            ...
            if unexpected:
                print(
                    f"Warning: {len(unexpected)} unexpected keys in checkpoint. Example: {unexpected[:5]}"
                )
        except Exception as e:
        ...
        if ckpt_path:
            try:
                ...
                total_params = len(list(model.state_dict().keys()))
                print(
                    f"Loaded checkpoint from MDX23C_CKPT: {ckpt_path} | loaded {num_loaded}/{total_params} tensors"
                )
                if missing:
                    print(
                        f"Warning: {len(missing)} missing keys (config mismatch likely). Example: {missing[:5]}"
                ...
                print(
                    f"Warning: failed to load checkpoint from MDX23C_CKPT {ckpt_path}: {e}"
                )
...
def demix_kit_from_mix(
    ...
    model, config, device, _, _ = load_model_from_hub(
        "kit_from_mix", device, force_download
    )
...
def demix_stems_from_kit(
    ...
    model, config, device, _, _ = load_model_from_hub(
        "stems_from_kit", device, force_download
    )
